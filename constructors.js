...
function User(name, age, email, savedPosts) {
  (this.name = name), (this.age = age), (this.email = email), (this.savedPosts = savedPosts);
}
...
User.prototype.removeSavedPost = function (id) {
  this.savedPosts.splice(
    this.savedPosts.findIndex((post) => post.id === id),
    1
  );
};
...
User.prototype.changePostRating = function (id, newRating) {
  this.savedPosts[this.savedPosts.findIndex((post) => post.id === id)].rating = newRating;
};
